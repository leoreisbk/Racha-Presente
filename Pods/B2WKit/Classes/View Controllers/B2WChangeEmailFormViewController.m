//
//  B2WChangeEmailFormViewController.m
//  B2WKit
//
...
#import "B2WChangeEmailFormViewController.h"
...
@interface B2WChangeEmailFormViewController ()

@property (nonatomic, strong) IBOutlet UITextField *currentEmailTextField;

@property (nonatomic, strong) IBOutlet UITextField *emailNewTextField;
@property (nonatomic, strong) IBOutlet UITextField *emailConfirmTextField;
...
@implementation B2WChangeEmailFormViewController

- (void)viewDidLoad {
    [super viewDidLoad];
	
	self.formElementGroups = @[@[[B2WFormElement formElementWithKey:@"currentId" textField:self.currentEmailTextField error:@""]],
							   @[[B2WFormElement formElementWithKey:@"newId" textField:self.emailNewTextField error:@""],
								 [B2WFormElement formElementWithKey:@"newIdConfirmation" textField:self.emailConfirmTextField error:@""]],
							   @[[B2WFormElement formElementWithKey:@"password" textField:self.passwordTextField error:@""]]];
	
	[self.currentEmailTextField setText:[B2WAPIAccount username]];
}
...
- (void)updateUserID
{
	...
	NSString *newID = self.emailNewTextField.text;
	...
	void (^completion)(id, NSError *) = ^(id object, NSError *error) {
		...
		// if ([localizedDescription isKindOfClass:[NSDictionary class]])
		{
			if (localizedDescription && (! [localizedDescription.allKeys containsObject:@"validationErrors"])
				&& [localizedDescription.allKeys containsObject:@"message"]) // erro do servidor
			{
			...
			else // é possível ter erro de validação
			{
				...
				{
					[B2WAPIAccount setUsername:newID];
					
					[self.navigationController popViewControllerAnimated:YES];
					
					[UIAlertView showAlertViewWithTitle:@"Email alterado com sucesso!"];
				}
	...
	[B2WAPIAccount updateUsernameForUserNamed:username newUsername:newID block:completion];
}
...
- (void)showValidationErrors:(NSArray *)errors
{
	for (NSDictionary *error in errors)
	{
		...
		if ([fieldKey isEqualToString:@"login"])
		{
			fieldKey = @"newId";
		}
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
	...
	if (textField.text.length > 0)
	{
		...
		{
			if (![self.passwordTextField.text isEqualToString:[B2WAPIAccount password]])
			{
		...
		{
			if (![self.emailNewTextField.text isEqualToString:self.emailConfirmTextField.text])
			{
				[self setErrorWithKey:@"newIdConfirmation" message:@"Emails não conferem." overwriteCurrentError:YES];
			}
		}
	}
}
